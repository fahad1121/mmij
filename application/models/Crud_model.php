...
class Crud_model extends CI_Model
{
    ...
    function select_html($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='single')
    {
        ...
        $return = '<select name="' . $name . '" onChange="' . $onchange . '(this.value,this)" class="form-control-sm ' . $class . '" ' . $other .  'style="' . $style . ' data-placeholder="' . $phrase . '" tabindex="2" data-hide-disabled="true" >';
        if (!is_array($from)) {
            if ($condition == '') {
                if ($from == 'country')
				{
    ...
    function allchats($table,$limit,$start,$col,$dir)
    {
        $this->db->select('*', FALSE);
        $this->db->from($table);
        $this->db->limit($limit,$start)->order_by($col,$dir)->order_by('message_id',$dir);
        $query = $this->db->get();
    ...
    function chats_search($table,$limit,$start,$search,$col,$dir)
    {
        $this->db->select('*', FALSE);
        $this->db->from($table);
        $this->db->like($table.'_thread_id',$search)->or_like($table.'.message_from',$search)->or_like($table.'.message_to',$search)->or_like($table.'.message_text',$search)->limit($limit,$start)->order_by($col,$dir);
        $query = $this->db->get();
    ...
    function chats_search_count($table,$search)
    {
        $this->db->select('*', FALSE);
        $this->db->from($table);
        $this->db->like($table.'_thread_id',$search)->or_like($table.'.message_from',$search)->or_like($table.'.message_to',$search)->or_like($table.'.message_text',$search);
        $query = $this->db->get();
    ...
    function message_thread_info($thread_id){
        return $this->db->get_where('message_thread',array('message_thread_id'=>$thread_id))->row();
    }
    ...
    function get_direct_messages_by_id($type, $type_id = '', $field = 'name')
    {
        if ($type_id != '') {
            $l = $this->db->get_where($type, array(
                $type . '_id' => $type_id
    ...
    function countMessage($member_id)
    {
        $counter = $this->db->query("select COUNT(message_thread_id) AS messageCount from message_thread WHERE ((message_to_seen IS NULL OR message_to_seen = '') AND message_thread_to = $member_id) OR ((message_from_seen IS NULL OR message_from_seen = '') AND message_thread_from = $member_id)");
        if($counter) {
            $rec = $counter->result_array();
            return $rec[0]['messageCount'];
        }else {
            return 0;
        }
