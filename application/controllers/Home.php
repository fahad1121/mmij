...
class Home extends CI_Controller
{
    ...
    function enable_message($member_id)
    {
        ...
        $member = $this->session->userdata('member_id');
        $direct_messages = $this->Crud_model->get_direct_messages_by_id('member', $member, 'direct_messages');
    ...
    function send_message($message_thread_id, $message_from, $message_to)
    {
        ...
        $threadInfo = $this->Crud_model->message_thread_info($message_thread_id);

        if ($threadInfo->message_thread_from == $message_to) {
            $updateData['message_from_seen'] = '';
            $updateData['message_to_seen'] = 'yes';
            $updateData['message_thread_time'] = time();
            $updateData['message_thread_timezone'] = gmdate('Y-m-d\TH:i:s.') . '000Z';


        } else if ($threadInfo->message_thread_from == $message_from) {
            $updateData['message_from_seen'] = 'yes';
            $updateData['message_to_seen'] = '';
            $updateData['message_thread_time'] = time();
            $updateData['message_thread_timezone'] = gmdate('Y-m-d\TH:i:s.') . '000Z';
        }

        $this->db->where('message_thread_id', $message_thread_id);
        $this->db->update('message_thread', $updateData);

        $this->trigger_event($message_thread_id);
        recache();
    }
...
//            echo '<pre>';print_r($page_data['get_member'][0]->ethnic_group);die;
            $this->load->view('front/index', $page_data);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $getEducationAndCareerJson = $this->db->select("education_and_career")->get_where("member", array("member_id" => $this->session->userdata('member_id')))->result();
                $getEducationAndCareerJson = str_replace("[", "", $getEducationAndCareerJson[0]->education_and_career);
                $getEducationAndCareerJson = str_replace("]", "", $getEducationAndCareerJson);
            ...
            $this->form_validation->set_rules('i_am_employed', 'I am employed', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                // ------------------------------------Education & Career------------------------------------ //
                $getEducationAndCareerJson = $this->db->select("education_and_career")->get_where("member",
                    array("member_id" => $this->session->userdata('member_id')))->result();
                $getEducationAndCareerJson = str_replace("[", "", $getEducationAndCareerJson[0]->education_and_career);
                $getEducationAndCareerJson = str_replace("]", "", $getEducationAndCareerJson);
                $dataForJson = json_decode($getEducationAndCareerJson, true);

                $dataForJson['highest_education'] = $dataForJson['highest_education'];
                $dataForJson['i_am_employed'] = $this->input->post('i_am_employed');
                $dataForJson['annual_income'] = $this->input->post('annual_income');
//                $dataForJson['my_job_title'] = $this->input->post('my_job_title');
                $dataForJson['my_company_name'] = $this->input->post('my_company_name');
                $dataForJson['years_worked'] = $this->input->post('years_worked');
                $dataForJson['self_employed'] = $this->input->post('self_employed');
                $dataForJson['years_owned'] = $this->input->post('years_owned');
                $dataForJson['annual_income_self'] = $this->input->post('annual_income_self');
                $json = json_encode($dataForJson);
                $data['education_and_career'] = $json;
                $this->db->where('member_id', $this->session->userdata('member_id'));
                $result = $this->db->update('member', $data);
        ...
        if ($this->member_permission() == TRUE) {
            ...
            if ($result) {
                if (!$result->is_email_verified) {
                    $this->session->set_flashdata('not_verified', 'To log in, please see email & activate your account');
                    redirect(base_url() . 'home/login', 'refresh');
                }
                    ...
                    if ($safe == 'yes') {
                        ...
                        if ($this->Crud_model->get_settings_value('third_party_settings', 'captcha_status', 'value') == 'ok') {
                            ...
                            if ($response['success']) {
                                ...
                                $displayMember = strtoupper(substr(md5(microtime()), rand(0, 26), 2)) . $insert_id;
                                ...
                                // $msg = 'done';
                                if ($this->Email_model->account_opening('member', $data['email'], $this->input->post('password'), $insert_id) == false) {
                                ...
                                // $msg = 'done';
                                if ($this->Email_model->new_account_notification($data['email'], $data['username'])) {
                                    //$msg = 'done_but_not_sent';
                                } else {
                                    //$msg = 'done_and_sent';
                                }

                                $this->session->set_flashdata('alert', 'To activate your account, please see email to confirm');
...
//            }
    ...
    public function email_verified()
    {
        ...
        $this->db->where('member_id', $encryptMemberId);
        $row = $this->db->update('member', $updateData);
    ...
    {
        $result = $this->db->query("SELECT member_id, member_since, email, first_name, last_name from member WHERE isProfileCompleted = 0")->result();
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            if ($days >= 3) {
                ...
                $this->Email_model->incomplete_profile_notification($result[$i]->email, $result[$i]->first_name . " " . $result[$i]->last_name);
            }
        }
    }
    ...
    public function trigger_event($threadID)
    {
        $this->load->library('ci_pusher');
        $pusher = $this->ci_pusher->get_pusher();

        // Set message
        $data['message'] = 'You got a message';
        $data['thread_id'] = $threadID;

        // Send message
        $event = $pusher->trigger('message_channel', 'my_event', $data);
    }
}