	 ...
	 */
                        ...
                        if ($para2 == "list_data") {
                                ...
                                if (!empty($members)) {
                                        foreach ($members as $key => $member) {
                                                ...
                                                $nestedData['name'] = "<span style='color:black;'>" . $member->first_name . ' ' . $member->last_name . "</span>";
                        ...
                        if (!empty($rows)) {
                                ...
                                foreach ($rows as $row) {
                                    ...
                                    if ($row->type == "profile_image") {
                                        ...
                                        $path = base_url()."uploads/profile_image/".$row->member_id."/".$profile_image;
                                    }
                                    elseif ($row->type == "gallery_image") {
                                        $path = base_url()."uploads/gallery_image/".$row->member_id."/".$row->item_name;
                                    }
                                    elseif ($row->type == "gallery_video") {
                                        $path = base_url()."video/".$row->member_id."/".$row->item_name;
                                    }
        ...
        function fakeMemberMessageReply()
        {
            ...
            foreach ($result as $key => $replyData) {
                ...
                // Send Reply
                $data['message_thread_id'] = $replyData->message_thread_id;
                $data['fake_message_id'] = $replyData->fake_message_id;
                $data['message_from'] = $replyData->message_from;
                $data['message_to'] = $replyData->message_to;
                $data['message_text'] = $replyData->message;
