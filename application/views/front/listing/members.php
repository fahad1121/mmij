<?php
// echo "<pre>";
// print_r($get_all_members);exit;
if (empty($get_all_members)) {
?>
    <div class='text-center pt-5 pb-5'><i class='fa fa-exclamation-triangle fa-5x'></i><h5><?=translate('no_results_found')?></h5></div>
<?php
}
...
if (intval($member->hide_profile) == 2){
    ...
    ?>
    <div class="block block--style-3 col-xl-4 col-lg-6 col-md-6 col-xs-12 list " style="float:left; border-radius: 2px; margin-bottom:22px !important"  id="block_<?=$member->member_id?>">
        <div class="block-image" style="padding-top: 19px;">
            <a onclick="return goto_profile(<?=$member->member_id?>)">
                    <?php
                    if (file_exists('uploads/profile_image/'.$member->member_id.'/'.$member->profile_image) && $member->is_profile_img_approved == 1) {
                    ?>
                    <?php
                    ...
                    ?>
                        <div class="listing-image" style="background-size:contain; background-image: url(<?=base_url()?>uploads/profile_image/default.jpg"></div>
                        <?php }elseif ($pic_privacy_data[0]['profile_pic_show']=='premium' and $is_premium>=2) {
                        ?>
                            <div class="listing-image" style="background-size:contain; background-image: url(<?=base_url()?>uploads/profile_image/<?=$member->member_id.'/'.$member->profile_image?>)"></div>
        ...
        <div class="block-title-wrapper">
            ...
            <table class="table-striped table-bordered table-sm" style="font-size: 12px;">
                <tr>
                    <td><b><?php echo strtoupper(translate('age'))?></b></td>
                    <td><?=strtoupper($calculated_age)?></td>
                </tr>
                <tr>
                    <td><b><?php echo strtoupper('Sect') ?></b></td>
                    <td><?=strtoupper($this->Crud_model->get_type_name_by_id('sect', $basic_info_data[0]['my_sect']));?></td>
                </tr>

                <tr>
                    <td><b><?php echo strtoupper('Profession') ?></b></td>
                    <td><?=strtoupper($this->Crud_model->get_type_name_by_id('profession', $basic_info_data[0]['profession']));?></td>
                </tr>
                <tr>
                    <td ><b><?php echo strtoupper('Residence') ?></b></td>
                    <td><div class="line-clamp"><?=strtoupper($this->Crud_model->get_type_name_by_id('country', $basic_info_data[0]['residence']));?></div></td>
                </tr>
                <tr>
                    <td ><b><?php echo strtoupper('Gender') ?></b></td>
                    <td>
                        <?php
                            $this->db->select('gender.name');
                            $this->db->from('gender');
                            $this->db->join('member', 'member.gender = gender.gender_id');
                            $this->db->where('member.gender', $member->gender);
                            $query = $this->db->get();
                            if (!empty($query->num_rows()))
                            {
                                //Use row() to get a single result
                                $gender = $query->row();
                                echo strtoupper($gender->name);
                            }
                        ?>
                    </td>
                </tr>
