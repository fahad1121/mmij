            ...
            <div class="row">
                <div class="col-md-4">
                    <?php if($get_member[0]->isOnline != 0 && $get_member[0]->isOnline > strtotime(date('Y-m-d H:i:s')) ){ ?>
                        <span style="float:left" class="online"> Online </span>
                    <?php }else{ ?>
                        <span style="float:left;color:#e91e63" class="offline">Offline</span>
                    <?php } ?>
                </div>
        <!--Removed code -->
            </div>
            <div class="col-md-12">
                <div class="my_class" style="display:block;text-align:center;"> <?= $get_member[0]->display_member;?></div>
            </div>
            <div class="profile-picture profile-picture--style-2" id="dp">
                ...
                $isProfileImgApproved = $get_member[0]->is_profile_img_approved;
                $images = json_decode($profile_image, true);
                $pic_privacy = $get_member[0]->pic_privacy;
                $pic_privacy_data = json_decode($pic_privacy, true);

                if (file_exists('uploads/profile_image/'.$get_member[0]->member_id.'/'.$get_member[0]->profile_image) && $isProfileImgApproved == 1) {
                    ...
                    <?php }elseif ($pic_privacy_data[0]['profile_pic_show']=='premium' and $membership >= 2) {
                        ?>
                        <div style="border: 10px solid rgba(255, 255, 255, 0.1);width: 200px;border-radius: 50%;margin-top: 10px;">
                            <div class="profile_img" id="show_img" style="background-image: url(<?=base_url()?>uploads/profile_image/<?=$get_member[0]->member_id.'/'.$get_member[0]->profile_image?>)"></div>
                        </div>
                    <?php }elseif ($pic_privacy_data[0]['profile_pic_show']=='premium' and $membership==1) {
                        ?>
                        <div style="border: 10px solid rgba(255, 255, 255, 0.1);width: 200px;border-radius: 50%;margin-top: 10px;">
                            <div class="profile_img" id="show_img" style="background-image: url(<?=base_url()?>uploads/profile_image/default.jpg)"></div>
                        </div>
            ...
            <div class="profile-useful-links clearfix mb-1">
                <!-- <div class="profile-details">
                    <h3 class="heading text-uppercase heading-6 strong-400 profile-occupation mt-3"><?=translate( 'Gallery')?></h3>
                </div> -->
                <?php
                $gallery_data = $this->db->get_where("gallery_items", array("member_id" => $get_member[0]->member_id))->result();
                ...
                if (!empty($gallery_data))
                {
                    ...
                    // $pic_privacy = $get_member[0]->pic_privacy;
                    // $pic_privacy_data = json_decode($pic_privacy, true);
                    // $is_premium_gallery = $this->Crud_model->get_type_name_by_id('member', $this->session->userdata('member_id'), 'membership');
                                ...
                                <div id="gallery" class="light-gallery">
                                    <div class="row mt-4">
                                        ...
                                        foreach ($gallery_data as $value)
                                        {
                                            if($value->is_approved == 1){
                                                if (file_exists('uploads/gallery_image/'.$get_member[0]->member_id.'/'.$value->item_name))
                                                {
                                                    if($pic_privacy_data[0]['gallery_show'] == 'only_me'){

                                                    }
                                                    elseif($pic_privacy_data[0]['gallery_show']=='premium' and $membership == 1)
                                                    {

                                                    }
                                                    elseif($pic_privacy_data[0]['gallery_show']=='premium' and $membership >= 2)
                                                    { ?>
                                                           <div class="col-sm-6 col-xs-12 mb-2">
                                                            <a target="_blank" href="<?=base_url()?>uploads/gallery_image/<?=$get_member[0]->member_id.'/'.$value->item_name?>" class="item">
                                                                <img src="<?=base_url()?>uploads/gallery_image/<?=$get_member[0]->member_id.'/'.$value->item_name?>" class="img-fluid rounded" style="height: 68px;">
                                                            </a>
                                                        </div>

                                                    <?php
                                                        }
                                                        elseif($pic_privacy_data[0]['gallery_show']=='all')
                                                        {
                                                    ?>
                                                               <div class="col-sm-6 col-xs-12 mb-2">
                                                            <a target="_blank" href="<?=base_url()?>uploads/gallery_image/<?=$get_member[0]->member_id.'/'.$value->item_name?>" class="item">
                                    ...
                                    <div class="row mt-2">
                                        <?php
                                            ...
                                            {
                                                if($value->is_approved == 1){
                                                    if (file_exists('video/'.$get_member[0]->member_id.'/'.$value->item_name))
                                                    {
                                                        $has_video = 1;

                                                            if($pic_privacy_data[0]['video_show'] == 'only_me')
                                                            {
                                                            }
                                                            elseif($pic_privacy_data[0]['video_show']=='premium' and $membership == 1)
                                                            {
                                                            }
                                                            elseif($pic_privacy_data[0]['video_show']=='premium' and $membership >= 2)
                                                            {
                                                        ?>
                                                                <div class="col-md-12 col-xs-12 mb-2">
                                                                    <video width="140" height="70" controls>

                                                                        <source src="<?php echo base_url()."video/".$get_member[0]->member_id.'/'.$value->item_name ?>">
                                                            ...
                                                            elseif($pic_privacy_data[0]['video_show']=='all')
                                                            {
                                                        ?>
                                                                <div class="col-md-12 col-xs-12 mb-2">
                                                                    <video width="140" height="70" controls>
            ...
            <div class="profile-connect mt-2">
                ...
                <div class="row" style="margin-top: 4px;">
                    <div class="col-sm-12 size-sm">
                        ...
                        <div class="profile-stats clearfix" style="padding-bottom: 12px;">
                            <div class="row">
                                <div class="stats-entry col-md-6" style="width: 100%">
                                    <span class="stats-label text-uppercase changeStatus"
                                          data-id= <?= $get_member[0]->member_id ?>>BLOCK PROFILE</span>
                                </div>
                                <div class="vl"></div>
                                <div class="stats-entry col-md-5" style="width: 100%">
                                        <span class="stats-label text-uppercase interestStatus"
                                              data-id= <?= $get_member[0]->member_id ?>>SAVE PROFILE</span>
                                </div>
                            </div>
                        </div>
                        <?php 
                            ...
                            if ( $logInMemberMembership != 4 || $otherMemberMembership != 4) {

                                $logInGender = $this->Crud_model->get_type_name_by_id('member', $this->session->userdata('member_id'), 'gender');

                            $otherGender = $this->Crud_model->get_type_name_by_id('member', $get_member[0]->member_id, 'gender');

                            if($logInGender != $otherGender) {
                        ?>
                        <a style="color:#000;box-shadow:none;" class="<?=$message_class?>" id="message_a_<?=$get_member[0]->member_id?>" <?php if ($message_onclick == 1){	?>onclick="return confirm_message(	<?=$get_member[0]->member_id?>)"<?php 	} elseif ($message_onclick == 0){ ?>onclick="threadViewableTo(<?php echo $if_message->message_thread_id?>);"<?php	}	?>>
...
<!--                </div>-->
        ...
        <div class="profile-stats clearfix mt-2">
            ...
            <?php if($basic_info_data[0]['residence']) { ?>
            <?php
            $sql ="SELECT name FROM country WHERE country_id =".$basic_info_data[0]['residence'];$query = $this->db->query($sql);
            if ($query->num_rows() > 0) { foreach ($query->result() as $row) {?> <?php  $countryName = $row->name; ?>  <?php }  } ?>
            <?php } ?>
            <div class="stats-entry ans">
                ...
                <span class="stats-label text-uppercase text-left pl-2"><?=isset($countryName) ? $countryName : ''?>&nbsp </span>
...
<script>
    ...
    $(document).ready(function () {
        $(document).on('click', '.changeStatus', function () {
            ...
            $.ajax({
                method: 'post',
                url: "<?=base_url()?>home/toggleBtn/"+by_member,
                data: {
                    by_member: by_member,
                    // _token: token
                },
    ...
    function threadViewableTo(thread_id){

        var thread_viewable_to = 1;
        $.ajax({
            type: "POST",
            url: "<?=base_url()?>home/threadViewableTo/"+thread_viewable_to+"/"+thread_id,
            cache: false,
            dataType: 'json',
            success: function(response) {
                profile_load('messaging',response);
    ...
    function profile_load(page,thread_id){

        var url = window.location.pathname;
        var id = url.substring(url.lastIndexOf('/') + 1);
        window.location.href = "<?=base_url()?>messaging/chat/"+id;
}
</script>
<style>
    /* xs */
