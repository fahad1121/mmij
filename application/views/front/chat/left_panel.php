...
// print_r($basic_info_data); die;

$memberProfile = $this->db->get_where("member", array("member_id" => $get_member[0]->member_id))->row();
if($memberProfile->gender == 1) {
    $memberBgClass = 'sidebar sidebar-inverse sidebar--style-1 bg-base-blue z-depth-2-top';
}else {
    $memberBgClass = 'sidebar sidebar-inverse sidebar--style-1 bg-base-1 z-depth-2-top';
}
...
<div class="sidebar sidebar-inverse sidebar--style-1 bg-base-1 z-depth-2-top">
    <div class="sidebar-object mb-4">
        ...
        <div id="container">
            <div class="row">
                <div class="col-md-4" id="member_online">
...
<!--               Removed code from here-->
            ...
            <div class="col-md-12">
                <div class="my_class" style="display:block;text-align:center;" id="member_name"> <?= $get_member[0]->display_member;?></div>
            </div>
                                        ...
                                        foreach ($gallery_data as $value) {
                                            if($value->is_approved == 1){
                                                if (file_exists('uploads/gallery_image/'.$get_member[0]->member_id.'/'.$value->item_name)) 
                                                {
                                                    ...
                                                    elseif($pic_privacy_data[0]['gallery_show']=='premium' and $is_premium == 1)
                                                    {

                                                    }
                                                    elseif($pic_privacy_data[0]['gallery_show']=='premium' and $is_premium >= 2)
                                                    {
                                                ?>
										...
										<?php
                                            ...
                                            if($value->is_approved == 1){
    											if (file_exists('video/'.$get_member[0]->member_id.'/'.$value->item_name))
                                                {
                                                    ...
                                                    elseif($pic_privacy_data[0]['video_show']=='premium' and $is_premium == 1)
                                                    {

                                                    }
                                                    elseif($pic_privacy_data[0]['video_show']=='premium' and $is_premium >= 2)
                                                    {
    										?>
                        ...
                        <div class="profile-stats clearfix" style="padding-bottom: 12px;">
                            <div class="row">
                                ...
                                <div class="stats-entry col-md-5" style="width: 100%">
                                    <span class="stats-label text-uppercase interestStatus interestEmail"
...
<script>
    ...
    $(document).ready(function () {
        $(document).on('click', '.interestEmail', function () {
            // var token = $('meta[name="csrf-token"]').attr('content');
            var to_member = $(this).data('id');
            console.log(to_member);
            ...
            $.ajax({
                method: 'post',
                url: "<?=base_url()?>home/interested/"+to_member,
                data: {
                    member: to_member,
                    // _token: token
                },
    ...
    $(document).ready(function () {
        $(document).on('click', '.interestStatus', function () {
            $(".interestStatus").fadeOut(function () {
                ...
                $.ajax({
                    method: 'post',
                    url: "<?=base_url()?>home/toggleInterest/"+to_member,
                    data: {
                        to_member: to_member,
                        // _token: token
                    },
