					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<div class="input-group">
								<?php 
									$system_currency = $this->db->get_where('business_settings', array('type' => 'currency'))->row()->value;
									$symbol = $this->db->get_where('currency_settings', array('currency_settings_id' => $system_currency))->row()->symbol;
								?>
								<div class="input-group-addon"><?=currency('', 'def')?></div>
								<input type="text" class="form-control" name="amount" value="<?=$value->amount?>"<?php if($value->plan_id == '1') {?> readonly <?php }?>>
							</div>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="number" class="form-control" name="express_interest" value="<?=$value->express_interest?>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="direct_messages"><b><?php echo translate('direct_messages')?></b></label>
						<div class="col-sm-9">
							<input type="number" class="form-control" name="direct_messages" value="<?=$value->direct_messages?>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="photo_gallery"><b><?php echo translate('photo_gallery')?></b></label>
						<div class="col-sm-9">
							<input type="number" class="form-control" name="photo_gallery" value="<?=$value->photo_gallery?>">
						</div>
					</div>
					<div class="form-group">
